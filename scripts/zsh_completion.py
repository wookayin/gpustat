...
for action in actions:
    ...
    if action.nargs == "?":
        if action.choices:
            choices[metavar] = " ".join(filter(len, map(str, action.choices)))
        completion = "'\"$_" + metavar + "\"'"
        metavar = ""
    elif action.choices:
...
for metavar, choice in choices.items():
    template = template.replace("{{" + metavar + "_choices}}", choice)
